...
const getCurrencies = async (req, res) => {
  const SHOP_CURRENCY_TBL = "SHOP_DBF.dbo.SHOP_CURRENCY_TBL";
  ...
  try {
    ...
    const countResult = await pool.request().query(`
      SELECT COUNT(*) AS total FROM ${SHOP_CURRENCY_TBL}
    `);
    const totalCount = countResult.recordset[0].total;
    const totalPages = Math.ceil(totalCount / limit);

    const currenciesResult = await pool
      .request()
      .input("offset", sql.Int, offset)
      .input("limit", sql.Int, limit).query(`
        SELECT * FROM ${SHOP_CURRENCY_TBL}
        ORDER BY id
        OFFSET @offset ROWS
    ...
    res.json({
      ...
      total: totalCount,
      totalPages,
      result: currenciesResult.recordset,
    });
  } catch (err) {
  ...
  try {
    ...
    const currencyResult = await pool
      ...
      .input("currencyId", sql.Int, currencyId).query(`
        SELECT * FROM ${SHOP_CURRENCY_TBL} WHERE id = @currencyId`);
...
module.exports = { getCurrencies, getCurrency, createCurrency };
